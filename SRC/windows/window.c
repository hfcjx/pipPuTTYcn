...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	...
	//PuTTYAttach
	//set stdout and stderr as binary mode
	_setmode(_fileno(stdout), _O_BINARY);
	_setmode(_fileno(stderr), _O_BINARY);
	//create stdin read thread.
    stdin_input_new(term);
    //
    /*
                                ...
                                WPARAM wParam, LPARAM lParam)
{
	...
	//PuTTYAttach
	send_excharng_copyData_msg(hwnd,term);
	//
    switch (message) {
      case WM_TIMER:
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
	  ...
	  case WM_COPYDATA:
	  {
		  ...
		  recv_copyData_msg(hwnd, wParam, lParam, term);
		  return 1;
	  }  
	  case WM_DROPFILES: 
	  {
		  recv_dropfiles_msg(hwnd, wParam, lParam);
		  break;
	  }
	  //
      default:
...
static void process_clipdata(HGLOBAL clipdata, bool unicode)
{
    ...
    if (unicode) {
        ...
        if (p) {
			...
			if (*p != 0x000d)
			{
				//Paste data to term,treat as pressing a space button
				key_press(wgs.term_hwnd, VK_SPACE);
			}
			//
...
{
	//PuTTYAttach
	return std_win_seat_output(term, data, len);
